...
import { GripVertical, Star, Heart, ThumbsUp, Image as ImageIcon } from 'lucide-react';
...
const QuestionPreview: React.FC<QuestionPreviewProps> = ({ question }) => {
  const isText = question.type === 'متنی';
  const isNumber = question.type === 'عددی';
      ...
      <div className="space-y-4">
        <div>
          <Label className="text-base font-medium text-gray-900">
            {question.label}
            {(question.required || question.isRequired) && <span className="text-red-500 mr-1">*</span>}
          </Label>
              ...
              <div className="flex gap-2">
                {Array.from({ length: question.ratingMax || 5 }, (_, i) => (
                  <button key={i} className="text-gray-300 hover:text-yellow-400 disabled:cursor-not-allowed" disabled>
                    {question.ratingStyle === 'heart' && <Heart className="w-6 h-6" />}
                    {question.ratingStyle === 'thumbs' && <ThumbsUp className="w-6 h-6" />}
                    {(!question.ratingStyle || question.ratingStyle === 'star') && <Star className="w-6 h-6" />}
                  </button>
                ))}
              </div>
            )}
            ...
            {isMatrix && (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse border border-gray-300">
                  ...
                  <tbody>
                    {(question.rows || ['سطر ۱', 'سطر ۲']).map((row, rowIndex) => (
                      <tr key={rowIndex}>
                        <td className="border border-gray-300 p-2 bg-gray-50 text-sm">{row}</td>
                        {(question.columns || ['ستون ۱', 'ستون ۲']).map((_, colIndex) => (
